...
if ! consteval {...}\end{lstlisting}
		...
		\begin{alertblock}{Attention}
			Les accolades sont obligatoires, même avec une unique instruction
		\end{alertblock}
...
\note[item]{Comparaison \textit{case-sensitive} comme toutes les fonctions de recherche dans des \lstinline|std::string|}
		...
		\item Erreur de compilation sur la construction de \lstinline|std::string| depuis \lstinline|nullptr|

\note[item]{Avant c'était un comportement indéfini}
	\end{itemize}
\end{frame}

\begin{frame}[fragile]
	\frametitle{Généralisation \lstinline|starts_with| et \lstinline|ends_with|}
	\begin{itemize}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item Automatise les appels à \lstinline|reset()| et \lstinline|release()| des \textit{smart pointers}
		\item \textit{Exception-safe} : smart pointer rétablie au retour de l'API C

\note[item]{Pas possible de lever une exception dans le code C++ avant de rétablir le \textit{smart pointer}}
		...
		\item \lstinline|inout_ptr| supporte la libération par l'API C
	\end{itemize}
\end{frame}
