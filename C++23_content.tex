...
\begin{frame}[fragile]
	\frametitle{Évolutions de char8\_t}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Relâchement des contraintes de wchar\_t}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{operator[] multidimensionnel}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::invoke\_r()}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Paramètre this explicite / deducing this}
	\begin{itemize}
		\item Objectif : limiter les surcharges \mintinline{cpp}|const| / non \mintinline{cpp}|const| de fonctions membres
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::pair}}
	\begin{itemize}
...
\note[item]{Code auparavant valide mais inefficace}
	...
	\begin{itemize}
		\item Construction depuis \mintinline{cpp}|std::tuple|, \mintinline{cpp}|std::array| ou un \textit{tuple-like}
	\end{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::stack \& std::queue}}
	\begin{itemize}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\begin{itemize}
				\item Interface similaire à \mintinline{cpp}|std::map| et \mintinline{cpp}|std::multimap|
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::mdspan}}
	\begin{itemize}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item Correction de \mintinline{cpp}|iterator_category| et \mintinline{cpp}|counted_iterator|
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::byteswap()}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Évolutions de {\NoAutoSpacing std::format}}
	\begin{itemize}
		\item Ajout du concept \mintinline{cpp}|formatable|
		\item Vérification des chaînes de format au \textit{compile-time}
		\item Réduction de la taille du binaire de \mintinline{cpp}|format_to|
		\item Support des types \mintinline{cpp}|std::generator|\textit{-like} par \mintinline{cpp}|std::format|
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item Formateur pour \mintinline{cpp}|std::vector<bool>::reference|
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::print}}
	\begin{itemize}
		\item Écriture dans \mintinline{cpp}|std::cout| depuis une chaîne de format \mintinline{cpp}|std::format|
	\end{itemize}
...
\begin{frame}[fragile]
	\frametitle{out\_ptr et inout\_ptr}
	\begin{itemize}
...
\note[item]{Pas d'exception dans le code C++ avant le rétablissement du \textit{smart pointer}}
		...
		\end{itemize}
		\item \mintinline{cpp}|out_ptr| permet à l'API C de modifier l'adresse contenu dans le smart pointer sans l'utiliser
		\item \mintinline{cpp}|inout_ptr| permet à l'API C de modifier l'adresse contenu dans le smart pointer mais aussi de l'utiliser
	\end{itemize}
\end{frame}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::unreachable()}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing time\_point::clock}}
	\begin{itemize}
...
\subsection*{Optional}
\begin{frame}[fragile]
	\frametitle{Opérations monadiques de {\NoAutoSpacing std::optional}}
	\begin{itemize}
...
\subsection*{Expected}
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::expected}}
	\begin{itemize}
		\item Classe template \mintinline{cpp}|std::expected<T, E>| contenant
		\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::unexpected}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Modules}
	\begin{itemize}
		\item Module \mintinline{cpp}|std| pour tout le namespace \mintinline{cpp}|std| depuis les en-têtes C++ et C
		\item Module \mintinline{cpp}|std.compat| : module \mintinline{cpp}|std| et le namespace global des en-têtes C
	\end{itemize}
\end{frame}

\subsection*{Coroutines}
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::generator}}
	\begin{itemize}
