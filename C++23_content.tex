...
\begin{frame}[fragile]
	\frametitle{\mintinline[style=white]{cpp}|std::pair|}
	\begin{itemize}
...
\note[item]{Pas d'exception dans le code C++ avant le rétablissement du \textit{smart pointer}}
		...
		\end{itemize}
		\item \mintinline{cpp}|std::out_ptr| permet à l'API C de modifier l'adresse contenu dans le smart pointer sans l'utiliser
		\item \mintinline{cpp}|std::inout_ptr| permet à l'API C de modifier l'adresse contenu dans le smart pointer mais aussi de l'utiliser
	\end{itemize}
\end{frame}
