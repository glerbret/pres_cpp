...
\note[item]{Plus sûr : \textit{type-safe} et \textit{resource-safe} - meilleur typage, RAII, etc.}
...
\note[item]{En C++, \lstinline|NULL| est un \lstinline|define| sur \lstinline|0| (ou équivalent), ne peut pas être \lstinline|(void*)0|}
...
\note[item]{Ces deux optimisations sont autorisées même lorsque le comportement observable est modifié}
		\end{block}
...
\note[item]{Les deux termes se trouvent. Notamment car c'est une forme simple (par rapport à des langages fonctionnels comme Caml ou Haskell) de l'inférence.}
...
\note[item]{La distinction \textit{run-time} / \textit{compile-time} est globalement compatible, au moins à première vue, avec l'explication au dessus mais est à mon sens trop adhoc et surtout laisse imaginer que typage dynamique = interprété et typage statique = compilé et que l'on ne peut faire aucune vérifications statique de types en typage dynamique ce qui n'est pas totalement vrai (voir linter)}
		\end{itemize}
	\end{block}
\end{frame}
...
\note[item]{\lstinline|noexcept| si l'exception est évaluée à \lstinline|true|}
	\end{itemize}
...
\note[item]{Usage particulier qui nous mènerait trop loin, rare et que je ne connais que très peu}
	\end{block}
...
\begin{frame}[fragile]
	...
	\begin{alertblock}{Attention}
		\lstinline|lock()| d'un mutex verrouillé par le même thread provoque un \textit{deadlock}
	\end{alertblock}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item \ldots{}en ne produisant aucun \textit{deadlock}
	\end{itemize}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\end{itemize}
		\item \lstinline|std::future| permet de récupérer une valeur disponible ultérieurement
		\begin{itemize}
