...
\note[item]{Dans le suite de la présentation, pas de séparation entre langage et bibliothèque standard}
\note[item]{Pas d'ordre particulier dans la présentation : enchaînement logique et tentative de regroupement par famille}
\note[item]{Pas complet, je laisse de côté system\_error, codecvt, typeindex et deux-trois nouvelles fonctions dans la bibliothèque standard (arcXXX trigo, is\_blank de locale)}
	\end{itemize}
\end{frame}
...
\note[item]{Là encore, class et struct sont totalement équivalents}
...
\note[item]{Contrairement au POD C++98, les données membres non-statiques peuvent donc être privée (ou protégées) du moment qu'elles le sont toutes}
...
\note[item]{En résumé : organisation mémoire similaire aux structures C}
...
\note[item]{Enfin erreur si taille des entiers différente de 3. Ce qui est très probable.}
...
\note[item]{Vérification \textit{compile-tim}e : typez correctement, définissez \lstinline|const| ce qui l'est par construction, pas de cast implicite (et donc constructeurs explicites), garantissez les invariants par construction si possible}
...
\note[item]{Les deux étant partiellement lié : c'est compliqué ou dangeureux car on a du mal à définir clairement ce qu'est cette copie}
...
\begin{frame}
	...
	\begin{center}
		\includegraphics[height=1\textheight]{input_src/C++/copie.png}
	\end{center}
\end{frame}
...
\begin{frame}
	...
	\begin{center}
		\includegraphics[height=1\textheight]{input_src/C++/deplacement.png}
	\end{center}
\end{frame}
...
\note[item]{Pour faire simple un fonction \lstinline|=default| est \textit{user-declared} mais pas \textit{user-provided}. Dans le cas de la génération implicite des opérations de déplacement, c'est bien le \textit{user-declared} qui désacive.}
				\end{itemize}
			\end{alertblock}
		\end{itemize}
	\end{itemize}
...
\begin{frame}
	...
	\begin{block}{\textit{Rule of five}}
		Les classes respectant la \textit{Rule of three} devraient définir également constructeur et opérateur d'affectation par déplacement
...
\note[item]{Contrairement à \textit{Rule of three}, l'absence des constructeur et opérateur d'affectation par déplacement n'est pas une erreur grave, mais une optimisation manquée}
	...
	\begin{block}{\textit{Rule of zero}}
		Classes sans gestion explicite d'\textit{ownership} : aucune de ces cinq fonctions

\note[item]{C'est à dire sans membres qu'il faut explicitement libérés, fermés, \ldots}
\note[item]{Ce devrait être le cas par défaut.}
	\end{block}
\end{frame}
...
\note[item]{Utilisé dans les exemples de la partie précédente par souci de place}
\end{frame}
...
\note[item]{Premier accroc dans les caractéristiques du C++ 98}
	\end{itemize}
...
\note[item]{Contre-exemple le C est statiquement typé mais pas très fortement (conversions implicites dans tout les sens) et moins que ne l'est le Python pourtant dynamiquement typé}
...
\note[item]{Préférence libre sur membre : plus générique (à commencer par tableau C), coût à priori nul}
	\end{exampleblock}
\end{frame}
...
\note[item]{Similaire à \lstinline|std::find_if| avec le prédicat nié, mais probablement plus clair}
\end{frame}
...
\note[item]{Donc initialisation d'attribut si présente, sinon constructeur par défaut (ou non initialisation pour \lstinline|int|, pointeur, \lstinline|etc.|) et erreur si un membre ne peut pas être construit}
		\end{itemize}
	\end{itemize}
...
\note[item]{Pas de fonction avec la même signature : pas de fonction avec ce nom, paramètres différents, qualificateurs différents}
	\end{itemize}
...
\note[item]{Deuxième accroc aux caractéristiques C++98/03}
...
\note[item]{J'utilise bien \og préférez\fg{}, c'est un bon guide mais pas une règle absolue, il existe de nombreux cas où \lstinline|std::function| et \lstinline|std::bind()| sont de meilleurs choix}
...
\note[item]{Bien entendu, il est totalement déconseillé d'utiliser FILE en C++ (bonne pratique !)}
...
\note[item]{push\_back d'un pointeur brute n'est pas possible et emplace\_back peut échouer en laissant fuir le pointeur}
	\end{alertblock}
...
\note[item]{L'attribut entre double crochet}
...
\note[item]{Entiers au sens très large, y compris least, fast, max, size\_t, etc.}
