...
\begin{frame}[fragile]
	\frametitle{\textit{Export templates}}
	\begin{itemize}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\end{itemize}
		\item Types entiers le plus grand disponibles \lstinline|intmax_t| et \lstinline|uintmax_t|
	\end{itemize}
\end{frame}
...
\begin{frame}[fragile]
	\frametitle{Nouveaux types entiers}
	\begin{itemize}
		\item Entiers de \lstinline|N| bits \lstinline|int<N>_t| et \lstinline|uint<N>_t|
		\begin{itemize}
			\item \lstinline|N| = 8, 16, 32 ou 64
...
\note[item]{Complément à 2 : inversion de chaque bit (complément à 1) puis ajout de 1}
		...
		\end{itemize}
		\item Plus petits entiers d'au moins \lstinline|N| bits \lstinline|int_least<N>_t| et \lstinline|uint_least<N>_t|
		\item Plus rapides entiers d'au moins \lstinline|N| bits \lstinline|int_fast<N>_t| et \lstinline|uint_fast<N>_t|
		\item Entiers capables de contenir une adresse \lstinline|intptr_t| et \lstinline|uintptr_t|
		\begin{itemize}
			\item Convertibles en \lstinline|void*| avec une valeur égale au pointeur original
			\item Support optionnel
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item \lstinline|std::is_trivial|
			\item \lstinline|std::is_trivially_copyable|
			\item \lstinline|std::is_standard_layout|
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item Constructeurs ou destructeurs \lstinline|=default| autorisés
...
\begin{frame}[fragile]
	\frametitle{\textit{Uniform Initialization}}
	\begin{itemize}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\end{itemize}
		\item Versions non ordonnées de \lstinline|std::map|, \lstinline|std::set|, \ldots{}
...
foo.set();
foo.all();        // True
foo.to_ullong();  // 31\end{lstlisting}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item \lstinline|std::is_partitioned()| indique si un ensemble est partitionné
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item \lstinline|std::partition_point()| retourne le point de partition d'un ensemble partitionné
		\begin{itemize}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item \lstinline|std::is_sorted()| indique si l'ensemble est ordonnée (ordre ascendant)
	\end{itemize}
...
\begin{frame}[fragile]
	\frametitle{\textit{Range-based for loop}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{\textit{User-defined literals}}
	\begin{itemize}
...
auto foo = [] { return 5; };\end{lstlisting}
	...
	\begin{alertblock}{Exception}
		\begin{itemize}
			\item Omission impossible si la lambda est \lstinline|mutable|
		\end{itemize}
	\end{alertblock}
...
using Bar = Foo<int, U, 5>;\end{lstlisting}
	...
	\begin{block}{\lstinline|using| de types}
		\begin{itemize}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item Constante \textit{compile-time} \lstinline|std::integral_constant|
		\item \lstinline|std::integral_constant| booléen vrai \lstinline|true_type|
		\item \lstinline|std::integral_constant| booléen faux \lstinline|false_type|
	\end{itemize}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item Collabore avec \lstinline|std::shared_ptr|
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item \lstinline|std::ratio_add()|, \lstinline|std::ratio_substract()|
			\item \lstinline|std::ratio_multiply()|, \lstinline|std::ratio_divide()|
		\end{itemize}
...
cout << r.num << "/" << r.den;   // 13/2\end{lstlisting}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item \lstinline|std::ratio_equal()|, \lstinline|std::ratio_not_equal()|
			\item \lstinline|std::ratio_less()|, \lstinline|std::ratio_less_equal()|
			\item \lstinline|std::ratio_greater()| et \lstinline|std::ratio_greater_equal()|
		\end{itemize}
	\end{itemize}
...
foo.count() * milliseconds::period::num / 
              milliseconds::period::den;  // 10\end{lstlisting}
\end{frame}
...
\begin{frame}[fragile]
	\frametitle{Horloges}
	\begin{itemize}
		\item Horloge temps-réel du système \lstinline|std::chrono::system_clock|
...
\begin{frame}[fragile]
	\frametitle{Horloges}
	\begin{itemize}
		\item Horloge monotone de mesure des intervalles \lstinline|std::chrono::steady_clock|
...
\begin{frame}[fragile]
	\frametitle{Horloges}
	\begin{itemize}
		\item Horloge avec le plus petit intervalle entre deux \textit{ticks} \lstinline|std::chrono::high_resolution_clock| 
...
cout << foo;        // Affiche 5 15\end{lstlisting}
	...
	\begin{itemize}
		...
		\item \lstinline|fetch_and()| et binaire et retour de la valeur avant modification
		\item \lstinline|fetch_or()| ou binaire et retour de la valeur avant modification
		\item \lstinline|fetch_xor()| ou exclusif et retour de la valeur avant modification
	\end{itemize}
\end{frame}
...
regex_search(string("abcdefg"), r);     // false\end{lstlisting}
	...
	\begin{itemize}
		\item \lstinline|std::regex_match()| vérifie la correspondance
	\end{itemize}
