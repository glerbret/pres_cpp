...
\note[item]{Différences entre le standard et ce \textit{draft} : des corrections éditoriales}
...
\note[item]{Globalement bon support de tous les compilateurs, à l'exception de Digital Mars}
...
\note[item]{Suppression de la contrainte \og Pas d'initialisation \textit{brace-or-equal-initializers} des données membres non-statiques\fg{} en C++14}
...
\note[item]{\lstinline|std::move()| force la sémantique de déplacement sur l'objet}
		...
		\begin{block}{\lstinline|std::forward()| ?}
			\begin{itemize}
				\item Objectif : \textit{perfect forwarding} (Voir \href{http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2002/n1385.htm}{N1385})
			\end{itemize}
		\end{block}
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{block}{Génération implicite}
		\begin{itemize}
			\item Pas de constructeur par copie, d'opérateur d'affectation, de destructeur, ni l'autre \og opérateur\fg{} de déplacement \textit{user-declared}
		\end{itemize}
	\end{block}
...
\note[item]{C statiquement typé mais pas très fortement (conversions implicites), moins que Python pourtant dynamiquement typé}
...
\note[item]{Distinction \textit{run-time} / \textit{compile-time} globalement compatible mais laisse penser que typage dynamique = interprété et typage statique = compilé et que l'on ne peut faire aucune vérifications statique de types en typage dynamique ce qui n'est pas totalement vrai (voir linter)}
	\end{block}
\end{frame}
...
\begin{frame}[fragile]
	\frametitle{\lstinline|auto|\titlehfill{}5/5}
	\begin{itemize}
...
\note[item]{Un cas où la compatibilité ascendante entre C++98/03 et C++11 a été rompue}
	...
	\begin{block}{Dépréciation}
		\begin{itemize}
			\item Mot-clé \lstinline|register| également déprécié
		\end{itemize}
...
\note[item]{Similaire à \lstinline|std::find_if| avec le prédicat nié, sémantiquement plus clair}
\end{frame}
...
\note[item]{Motivation : gestion d'indice en plusieurs endroits plus complexe à lire et source d'erreur sur celui-ci (incrémentation) mais garantie de ne pas dépasser les bornes ce qui est une erreur bien moins évidente à détecter en test et plus dangereuse}
		\end{itemize}
	\end{block}
...
\note[item]{Règle historique liée : initialisez dans la liste d'initialisation et non dans le corps du constructeur}
	\end{exampleblock}
...
\note[item]{Fonctions de conversions manuelles, \lstinline|static_cast| pour le type mais il n'y a pas vraiment de vérification. Des tentatives, plutôt réussies pour certaines, à coup de \lstinline|template| ou de \lstinline|define|}
	\end{block}
\end{frame}
...
for(int i=0; i<5; ++i) foo[i] = i;\end{lstlisting}
	...
	\begin{block}{Dépréciation}
		\begin{itemize}
			\item Dépréciation de \lstinline|std::auto_ptr| au profit de \lstinline|std::unique_ptr|
		\end{itemize}
	\end{block}
\end{frame}
