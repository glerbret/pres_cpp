...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item Hérités de C99 (\mintinline{cpp}|cstdint| et \mintinline{cpp}|cinttypes|)
	\end{itemize}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item Fonctions de manipulation de \mintinline{cpp}|intmax_t| et \mintinline{cpp}|uintmax_t| (\mintinline{cpp}|imaxabs()|, \mintinline{cpp}|imaxdiv()|, \mintinline{cpp}|strtoimax()|, \mintinline{cpp}|strtoumax()|, \mintinline{cpp}|wcstrtoimax()| et \mintinline{cpp}|wcstrtoumax()|)
...
\note[item]{\mintinline{cpp}|std::move()| force la sémantique de déplacement sur l'objet}
		...
		\begin{block}{\mintinline{cpp}|std::forward()| ?}
			\begin{itemize}
...
\begin{frame}
	...
	\begin{alertblock}{\textit{user-declared} ? \textit{user-provided} ?}
		\begin{itemize}
			\item \textit{user-declared} : la fonction est déclarée par l'utilisateur, y compris en \mintinline{cpp}|=default| 
...
\begin{frame}[fragile]
\frametitle{{\NoAutoSpacing std::forward\_list}}
	\begin{itemize}
...
\begin{frame}[fragile]
	...
	\begin{block}{\mintinline[style=vim]{cpp}|foo.data()| ou \mintinline[style=vim]{cpp}|&foo[0]| ?}
		\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::string}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::bitset}}
	\begin{itemize}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item Fonctions membres \mintinline{cpp}|cbegin()|, \mintinline{cpp}|cend()|, \mintinline{cpp}|crbegin()| et \mintinline{cpp}|rcend()| retournant des \mintinline{cpp}|const_iterator|
...
\begin{frame}[fragile]
	\frametitle{Itérateurs}
	\begin{itemize}
		\item Fonctions libres \mintinline{cpp}|std::prev()| et \mintinline{cpp}|std::next()| pour retrouver l'itérateur suivant ou précédent
		\item \mintinline{cpp}|move_iterator| : adaptateur d'itérateur retournant des \textit{rvalue reference} lors du déréférencement
	\end{itemize}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item \mintinline{cpp}|std::bit_and()| : et bit à bit
		\item \mintinline{cpp}|std::bit_or()| : ou inclusif bit à bit
		\item \mintinline{cpp}|std::bit_xor()| : ou exclusif bit à bit
	\end{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::string \& conversions}}
	\begin{itemize}
...
\subsection*{tuple}
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::tuple}}
	\begin{itemize}
	...
	\begin{block}{\mintinline{cpp}|std::make_tuple| ou constructeur ?}
		\mintinline{cpp}|std::make_tuple()| permet la déduction de types, pas le constructeur
...
\begin{frame}[fragile]
	...
	\begin{block}{\mintinline[style=vim]{cpp}|=default| ou non définition ?}
		\begin{itemize}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item \mintinline{cpp}|std::exception_ptr| quasi-pointeur à responsabilité partagée sur une exception
...
\begin{frame}[fragile]
	\frametitle{Énumérations fortement typées \mintinline{cpp}|enum class|}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::mem\_fn}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::bind}}
	\begin{itemize}
...
\begin{frame}[fragile]
	...
	\begin{alertblock}{Capture de \mintinline{cpp}|this|}
		\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{lambda, {\NoAutoSpacing std::function}, \ldots{} - Conclusion}
	\begin{exampleblock}{Do}
		\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::reference\_wrapper}}
	\begin{itemize}
...
\begin{frame}[fragile]
	...
	\begin{block}{\mintinline[style=vim]{cpp}|using| de types}
		\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::enable\_if}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Pointeurs intelligents -- {\NoAutoSpacing std::unique\_ptr}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Pointeurs intelligents -- {\NoAutoSpacing std::shared\_ptr}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Pointeurs intelligents -- {\NoAutoSpacing std::make\_shared()}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Pointeurs intelligents -- {\NoAutoSpacing std::weak\_ptr}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Variables atomiques -- {\NoAutoSpacing std::atomic}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Variables atomiques -- {\NoAutoSpacing std::atomic\_flag}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Threads -- {\NoAutoSpacing std::thread}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Threads -- {\NoAutoSpacing std::this\_thread}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Mutex -- {\NoAutoSpacing std::mutex}}
	\begin{itemize}
		\item Verrou pour l'accès exclusif à une section de code
		\item \mintinline{cpp}|lock()| verrouille le mutex (en attendant sa libération s'il est déjà verrouillé)
...
\begin{frame}[fragile]
	\frametitle{Mutex -- {\NoAutoSpacing std::timed\_mutex}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Mutex -- {\NoAutoSpacing std::lock\_guard}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Mutex -- {\NoAutoSpacing std::unique\_lock}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Mutex -- {\NoAutoSpacing std::call\_once()}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Variables conditionnelles -- {\NoAutoSpacing std::condition\_variable}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Futures \& promise -- {\NoAutoSpacing std::future}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Futures \& promise -- {\NoAutoSpacing std::async()}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Futures \& promise -- {\NoAutoSpacing std::promise}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Futures \& promise -- {\NoAutoSpacing std::packaged\_task}}
	\begin{itemize}
