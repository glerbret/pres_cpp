...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item Utilisation du mot clé \lstinline|inspect| (ou \lstinline|switch|) et du \textit{wildcard} \lstinline|__| (ou \lstinline|_|)
		\item Utilisable sur
		\begin{itemize}
			\item Entiers
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item \lstinline|std::is_detected| indique si un \textit{template-id} est bien formé
		\item Wrapper \lstinline|std::propagate_const| pour les pointeurs et \textit{pointer-like}
		\item Pointeurs intelligents non possédants \lstinline|std::observer_ptr|
		\item \lstinline|std::ostream_joiner| écrit des éléments dans un flux de sortie avec un séparateur
	\end{itemize}
...
// "1, 2, 3, 4, 5"\end{lstlisting}
	...
	\begin{itemize}
		\item Générateur aléatoire propre au thread \lstinline|std::default_random_engine| initialisé dans un état non prédictif
		\begin{itemize}
			\item \lstinline|std::randint()| génère un nombre entier dans une plage spécifiée
			\item \lstinline|std::reseed()| modifie la graine de génération
			\item \lstinline|std:sample()| choisit aléatoirement $n$ élément d'une séquence
			\item \lstinline|std::shuffle()| réordonne aléatoirement les éléments d'un range
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item appelé à la sortie du scope : \lstinline|std::scope_exit|
			\item appelé à la sortie du scope par une exception : \lstinline|std::scope_fail|
			\item appelé à la sortie du scope hors exception : \lstinline|std::scope_success|
		\end{itemize}
		\item \textit{RAII wrapper} \lstinline|std::unique_resource|
	\end{itemize}
\end{frame}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item Politique d'exécution \lstinline|vector_policy|
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item Versions de \lstinline|std::future| et \lstinline|std::shared_future| supportant les continuations
		\begin{itemize}
			\item \lstinline|is_ready()| indique si l'état partagé est disponible
			\item \lstinline|then()| attache une continuation à la future
		\end{itemize}
		\item \lstinline|std::when_any| crée une future disponible lorsqu'une des futures contenues devient disponible
		\item \lstinline|std::when_all| crée une future disponible lorsque toutes les futures contenues sont disponibles
		\item \lstinline|std::make_ready_future()| crée une future contenant une valeur immédiatement disponible
		\item \lstinline|std::make_exceptional_future()| crée une future contenant une exception immédiatement disponible
	\end{itemize}
\end{frame}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item Attributs \lstinline|[[optimize_for_synchronized]]|
	\end{itemize}
\end{frame}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item \lstinline|x.f(...)| tente d'appeler \lstinline|f(x, ...)| si \lstinline|x.f(...)| n'est pas valide
			\item \lstinline|p->f(...)| tente d'appeler \lstinline|f(p, ...)| si \lstinline|p->f(...)| n'est pas valide
			\item Si \lstinline|f(x, ...)| n'est pas valide, \lstinline|f(x, ...)| tente d'appeler
			\begin{itemize}
				\item \lstinline|x->f(...)| si \lstinline|operator->| existe pour \lstinline|x|
				\item \lstinline|x.f(...)| sinon
			\end{itemize}
