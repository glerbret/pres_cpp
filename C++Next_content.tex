...
\note[item]{Recouvre les points du TS \textit{contract} mais est plus ambitieux}
		...
		\item Amélioration de la compatibilité et de la portabilité de \lstinline|char8_t|

\note[item]{Permettre l'initialisation de table de \lstinline|char| ou \lstinline|unsigned char| depuis des chaînes littérales UTF-8 ou \lstinline|char8_t|}
...
\note[item]{Alignement sur les évolutions C en cours}
...
\note[item]{Saturation arithmetic : pas de débordement, on reste sur les valeurs min/max si elles sont dépassées lors d'une opération : 252 + 7 sur un char donne 255}
...
\note[item]{\lstinline|block_scope| : règle de durée de vie des littéraux C}
\note[item]{\lstinline|statement_scope| : règle sur la durée de vie des temporaires en C++}
