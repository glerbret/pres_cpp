...
\note[item]{Auparavant, ils pouvaient représenter ces caractères, mais ce n'était pas une obligation et pouvaient représenter d'autres codages de caractères larges. Ce n'est plus le cas}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item Possibilité de définir des spécifications d'exception des fonctions \lstinline|=default| différentes de celles de la fonction implicite
...
for(size_t i = 0; const auto& bar : foo) {
  ...
  ++i; }\end{lstlisting}
		...
		\begin{block}{Intérêt}
			Itération (via des fonctions libres) d'éléments ayant une fonction membre \lstinline|begin()| ou \lstinline|begin()| mais pas les deux
		\end{block}
...
\begin{frame}[fragile]
	\frametitle{Évolutions des templates\titlehfill{}1/2}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Évolutions des templates\titlehfill{}2/2}
	\begin{itemize}
...
\note[item]{Fourni au compilateur des information lui permettant des optimisations plus pertinentes}
			...
			\item \lstinline|[[no_unique_address]]| : indique qu'un membre non statique ne nécessite pas une adresse unique
