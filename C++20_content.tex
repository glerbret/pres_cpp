...
\begin{frame}[fragile]
	\frametitle{\mintinline[style=white]{cpp}|operator<=>|}
	\begin{itemize}
...
\note[item]{Similaire au fonctionnement de \mintinline{cpp}|strcmp|, \mintinline{cpp}|memcmp|, \ldots{}}
\note[item]{Un retour non numérique, p.ex. \mintinline{cpp}|Nan|, indique des valeurs non ordonnées}

		\end{itemize}
		\item Trois types de retour possibles
		\begin{itemize}
			\item \mintinline{cpp}|std::strong_ordering| : ordre et égalité
			\item \mintinline{cpp}|std::weak_ordering| : ordre et équivalence
...
\note[item]{On peut avoir équivalence et non égalité lorsqu'il y a plusieurs représentation d'une même valeur : comparaison sur certains champs seulement, codage des \mintinline{cpp}|float| et \mintinline{cpp}|double|}

		\item Peut être généré par le compilateur (\mintinline{cpp}|=default|)
		\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{\mintinline[style=white]{cpp}|operator<=>|}
	\begin{itemize}
		\item \mintinline{cpp}|operator<=>| déclenche la génération
		\begin{itemize}
			\item Opérateurs d'ordre (\mintinline{cpp}|<|, \mintinline{cpp}|<=|, \mintinline{cpp}|>| et \mintinline{cpp}|>=|) via \mintinline{cpp}|operator<=>|
			\item \mintinline{cpp}|operator==| via \mintinline{cpp}|operator==| des bases et membres
			\item \mintinline{cpp}|operator!=| via \mintinline{cpp}|operator==|
		\end{itemize}
	\end{itemize}

	\begin{alertblock}{\mintinline[style=white]{cpp}|operator==| et \mintinline[style=white]{cpp}|operator!=|}
		\begin{itemize}
			\item Uniquement pour les retours de type égalité
...
\note[item]{Initialement, \mintinline{cpp}|operator==| et \mintinline{cpp}|operator!=| aussi résolu via \mintinline{cpp}|operator<=>|. Modifié pour ne pas avoir des tests d'égalité non optimaux \og par erreur\fg{} via le \textit{fallback} sur \mintinline{cpp}|operator<=>| (p.ex. court-circuit sur la taille de conteneurs)}
	...
	\begin{itemize}
		\item Possible de marquer ces autres opérateurs \mintinline{cpp}|=default|

\note[item]{Mais non obligatoire pour les opérateurs d'ordre générés dès que \mintinline{cpp}|operator<=>| est présent. Mais \mintinline{cpp}|=default| permet de prendre leurs adresses}
\note[item]{\mintinline{cpp}|=default| utile pour générer \mintinline{cpp}|operator!=| ou \mintinline{cpp}|operator==| en l'absence de \mintinline{cpp}|operator<=>|}
\note[item]{Un opérateur \mintinline{cpp}|=default| qui ne peut pas être généré reste bien \textit{deleted}}
...
\begin{frame}[fragile]
	...
	\begin{block}{Pour aller plus loin}
		\begin{itemize}
			\item \href{http://site.icu-project.org/home}{\mintinline{cpp}|ICU|} supporte de nombreux calendriers et mécanismes de localisation
		\end{itemize}
