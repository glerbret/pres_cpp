...
- Ajout de ```cpp at()``` à ```cpp std::span```
- Ajout de ```cpp at()``` à ```cpp std::mdspan```
...
#noteblock("Note", "Paramètres et variables définies hors du contrat sont constants lors de la vérification des contrats")
