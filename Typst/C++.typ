...
\end{itemize}
\item ```cpp extract()``` extrait le nœud du premier conteneur
\begin{itemize}
\item Nœud identifié par sa clé ou par un itérateur
\item Retourne un \textit{node handle}
\end{itemize}
\item Surcharge de ```cpp insert()``` prenant un \textit{node handle} en paramètre
\begin{itemize}
\item Retourne une structure indiquant la réussite ou non de l'insertion
\item ... et, en cas d'échec, le \textit{node handle}
\end{itemize}
\end{itemize}
...
\item Modifier une clé dans une ```cpp std::map```
\end{itemize}
\end{block}

#addproposal{P0083}{https://wg21.link/P0083}
...
\begin{itemize}
\item ```cpp merge()``` fusionne le contenu de conteneurs associatifs
\end{itemize}
...
== \mintinline[style=white]{cpp}```std::map``` et \mintinline[style=white]{cpp}```std::unordered_map```}
\begin{itemize}
\item ```cpp try_emplace()``` tente de construire en place
\item ... sans effet, même pas un \og vol\fg{} de la valeur, si la clé existe déjà
\item ```cpp insert_or_assign()``` ajoute ou modifie un élément
\end{itemize}
...
== \mintinline[style=white]{cpp}```emplace_back()```, \mintinline[style=white]{cpp}```emplace_front()```}
\begin{itemize}
...
#noteblock{Note}
\begin{itemize}
\item ```cpp emplace()``` renvoie toujours un itérateur
\end{itemize}
\end{block}
...
#addproposal{P0084}{https://wg21.link/P0084}
...
\item ```cpp std::size()```
\begin{itemize}
\item Conteneurs et ```cpp initializer_list``` : résultat de la fonction membre ```cpp size()```
\item Tableau C : taille du tableau
\end{itemize}

\item ```cpp std::empty()```
\begin{itemize}
\item Conteneurs : résultat de la fonction membre ```cpp empty()```
\item Tableau C : ```cpp false```
\item ```cpp initializer_list``` : ```cpp size() == 0```
\end{itemize}

\item ```cpp std::data()```
\begin{itemize}
\item Conteneurs : résultat de la fonction membre ```cpp data()```
\item Tableau C : pointeur sur le premier élément
\item ```cpp initializer_list``` : itérateur sur le premier élément
\end{itemize}
\end{itemize}
...
\begin{frame}
== \mintinline[style=white]{cpp}```ContiguousIterator```}
\begin{itemize}
\item Basé sur ```cpp RandomAccessIterator```
\item Mais sur des conteneurs à stockage contigu
\item Itérateur associé à
\begin{itemize}
\item ```cpp std::vector```
\item ```cpp std::array```
\item ```cpp std::basic_string```
\item ```cpp std::valarray```
\item Aux tableaux C
\end{itemize}
\end{itemize}
...
\item Utilisation  de ```cpp memcpy``` et ```cpp memset```
\end{itemize}
\end{block}
...
\begin{itemize}
\item ```cpp std::clamp()``` ramène une valeur dans une plage donnée
\begin{itemize}
...
#addproposal{P0025}{https://wg21.link/P0025}



== \mintinline[style=white]{cpp}```std::to_chars()``` et \mintinline[style=white]{cpp}```std::from_chars()```}
\begin{itemize}
...
\note[item]{C'est à dire sur le caractères suivant le dernier écrit pour ```cpp to_chars``` ou le premier non converti pour ```cpp from_chars()```}

\item Et un code erreur
\end{itemize}

#alertblock{API bas-niveau}
\begin{itemize}
...
#addproposal{P0067}{https://wg21.link/P0067}


== \mintinline[style=white]{cpp}```std::variant```}
\begin{itemize}
...
\begin{itemize}
\item Ne peut pas contenir de références, de tableaux C, ```cpp void``` ni être vide
...
\note[item]{Mais peut contenir des ```cpp std::array<>``` (alternative aux tableaux C) et des ```cpp std::reference_wrapper<>``` (alternative aux références)}

\item ```cpp std::variant``` \textit{default-constructible} seulement si le premier type l'est

\note[item]{Le ```cpp std::variant``` contient alors le premier type initialisé par défaut}
\end{itemize}
\end{alertblock}

#noteblock{\mintinline[style=white]{cpp}```std::monostate```}
\begin{itemize}
\item Permet d'émuler des ```cpp std::variant``` vides

\note[item]{Les ```cpp std::variant``` vides peuvent être utile dans le cas de code template}

\item Rend un ```cpp std::variant``` \textit{default constructible}

\note[item]{Si ```cpp std::monostate``` est le premier élément du ```cpp std::variant```, celui-ci devient \textit{default-constructible}}
\end{itemize}
\end{block}

#adviceblock{Do}
\begin{itemize}
\item Préférez ```cpp std::variant``` aux unions brutes
\end{itemize}
\end{exampleblock}

#addproposal{P0088}{https://wg21.link/P0088}



== \mintinline[style=white]{cpp}```std::variant```}
\begin{itemize}
\item ```cpp get<>()``` récupère la valeur depuis l'index ou le nom du type
\item Et lève une exception si le type demandé n'est pas correct
\item ```cpp get_if<>()``` retourne un pointeur sur la valeur ou ```cpp nullptr```
\item ```cpp std::holds_alternative<>()``` teste le type contenu
\item ```cpp index()``` retourne l'index d'un type donnée
...
== \mintinline[style=white]{cpp}```std::variant```}
\begin{itemize}
\item ```cpp std::visit()``` permet l'appel sur le type réellement contenu
\end{itemize}
...
\begin{itemize}
\item Appelable valide pour tous les types du ```cpp std::variant```
\end{itemize}

\note[item]{Et ce peut être une structure/classe présentant une surcharge de ```cpp operator()``` pour chacun des types}
\end{alertblock}

#noteblock{En attendant C++17}
\begin{itemize}
\item Utilisez ```cpp Boost.Variant```
\end{itemize}
\end{block}
...
== Pack expansion sur \mintinline[style=white]{cpp}```using```}
\begin{itemize}
...
#addproposal{P0195}{https://wg21.link/P0195}
...
#addproposal{P0036}{https://wg21.link/P0036}
...
== Fold expression}
#alertblock{\textit{left fold} ou \textit{right fold} ?}
```cpp
		template<typename... Args>
...
\begin{itemize}
\item ```cpp true``` pour l'opérateur ```cpp &&```
\item ```cpp false``` pour l'opérateur ```cpp ```
\item ```cpp void()``` pour l'opérateur ```cpp ,```
\end{itemize}
\end{itemize}
...
\begin{itemize}
\item Y compris \og ```cpp ,```\fg{} qui va donner une séquence d'actions
\end{itemize}
...
\begin{itemize}
\item Utilisation possible de types différents pour ```cpp begin``` et ```cpp end```
\item Permet de traiter des paires d'itérateurs
\item ... mais aussi un itérateur et une taille
\item ... ou un itérateur et une sentinelle de fin
\item Compatible avec les travaux sur Range TS
\end{itemize}

#addproposal{P0184}{https://wg21.link/P0184}
...
#addproposal{P0136}{https://wg21.link/P0136}



== \mintinline[style=white]{cpp}```noexcept```}
\begin{itemize}
\item ```cpp noexcept``` fait partie du type des fonctions
\end{itemize}
...
\note[item]{Une surcharge sans ```cpp noexcept``` doit être ajoutée}

\begin{itemize}
\item Les fonctions ```cpp noexcept``` peuvent être convertie en fonctions non ```cpp noexcept```
\end{itemize}
...
#addproposal{P0012}{https://wg21.link/P0012}



== \mintinline[style=white]{cpp}```std::uncaught_exceptions()```}
\begin{itemize}
...
\begin{itemize}
\item Caractère UTF-8 préfixé par ```cpp u8```
...
#addproposal{P0091}{https://wg21.link/P0091}
...
== \mintinline[style=white]{cpp}```template <auto>```}
\begin{itemize}
...
#addproposal{P0127}{https://wg21.link/P0127}



== Template et contraintes d'utilisation}
\begin{itemize}
\item ```cpp typename``` autorisé dans les déclarations de \textit{template template parameters}
\end{itemize}
...
\note[item]{Auparavant, il fallait obligatoirement utiliser ```cpp class```, alors que ```cpp typename``` était valide partout ailleurs}
...
== Capture de \mintinline[style=white]{cpp}```*this```}
\begin{itemize}
\item Capture ```cpp *this``` par valeur
\end{itemize}
...
#addproposal{P0018}{https://wg21.link/P0018}
...
#addproposal{P0170}{https://wg21.link/P0170}



== Lambdas et expressions constantes}
\begin{itemize}
\item Déclaration ```cpp constexpr``` de lambda possible
\item Explicitement via ```cpp constexpr```
\end{itemize}
...
\begin{itemize}
\item Implicitement ```cpp constexpr``` lorsque les exigences sont satisfaites
\end{itemize}
...
== \mintinline[style=white]{cpp}```std::invoke()```}
\begin{itemize}
\item Appelle l'appelable fourni en paramètre
\item ... en fournissant la liste de paramètres
\item ... et en retournant le retour de l'appelable
\end{itemize}
...
== \mintinline[style=white]{cpp}```std::not_fn()```}
\begin{itemize}
...
\begin{itemize}
\item Dépréciation de ```cpp std::not1``` et ```cpp std::not2```
\end{itemize}
\end{block}
...
#addproposal{p0005}{https://wg21.link/p0005}
#addproposal{p0358}{https://wg21.link/p0358}



== Alias de traits}
\begin{itemize}
\item Ajout du suffixe ```cpp _v``` aux traits de la forme ```cpp is_...```
\item Suppression de ```cpp ::value```
\end{itemize}
...
#addproposal{P0006}{https://wg21.link/P0006}
...
\begin{itemize}
\item ```cpp is_swappable_with```, ```cpp is_swappable```, ```cpp is_nothrow_swappable_with``` et ```cpp is_nothrow_swappable``` : objets échangeables
\item ```cpp is_callable``` et ```cpp is_nothrow_callable``` : objet appelable
\item ```cpp void_t``` conversion en ```cpp void```
\end{itemize}
\item Méta-fonctions sur les traits
\begin{itemize}
\item ```cpp std::conjunction``` : et logique entre traits
\item ```cpp std::disjunction``` : ou logique entre traits
\item ```cpp std::negation``` : négation d'un trait
\end{itemize}
\end{itemize}
...
#addproposal{P0185}{https://wg21.link/P0185}
#addproposal{P0013}{https://wg21.link/P0013}



== Gestion des attributs}
\begin{itemize}
\item Usage étendu aux déclarations de ```cpp namespace```
\end{itemize}
...
\item ```cpp using``` des attributs non standards
\end{itemize}
...
#addproposal{P0283}{https://wg21.link/P0283}
#addproposal{P0028}{https://wg21.link/P0028}



== Attribut \mintinline[style=white]{cpp}```[[ fallthrough ]]```}
\begin{itemize}
\item Dans un ```cpp switch``` avant un ```cpp case``` ou ```cpp default```
...
#addproposal{P0188}{https://wg21.link/P0188}



== Attribut \mintinline[style=white]{cpp}```[[ nodiscard ]]```}
\begin{itemize}
...
#noteblock{Note}
\begin{itemize}
\item Conversion implicite en ```cpp void``` pour supprimer l'avertissement
\end{itemize}
...
#addproposal{P0189}{https://wg21.link/P0189}



== Attribut \mintinline[style=white]{cpp}```[[ nodiscard ]]```}
\begin{itemize}
\item ... ou sur un type (classe, structure ou énumération)
\end{itemize}
...
== Attribut \mintinline[style=white]{cpp}```[[ maybe_unused ]]```}
\begin{itemize}
\item Sur une classe, structure, fonction, variable, paramètre, ...
...
#addproposal{P0212}{https://wg21.link/P0212}
...
#noteblock{Au delà du compilateur}
\begin{itemize}
...
== \mintinline[style=white]{cpp}```std::shared_mutex```}
\begin{itemize}
\item Similaire à ```cpp std::mutex``` avec deux niveaux d'accès
\begin{itemize}
\item Exclusif : possible si le verrou n'est pas pris
\item Partagé : possible si le verrou n'est pas pris en exclusif
\end{itemize}
\item API identique à ```cpp std::mutex``` pour l'accès exclusif
\item API similaire pour l'accès partagé
\begin{itemize}
\item ```cpp lock_shared```
\item ```cpp try_lock_shared```
\item ```cpp unlock_shared```
\end{itemize}
\end{itemize}

#noteblock{Note}
\begin{itemize}
\item Équivalent \textit{non-timed} de ```cpp std::shared_timed_mutex```
\end{itemize}
\end{block}
...
== \mintinline[style=white]{cpp}```std::scoped_lock```}
\begin{itemize}
...
#addproposal{P0156}{https://wg21.link/P0156}



== \mintinline[style=white]{cpp}```std::apply()```}
\begin{itemize}
...
\begin{itemize}
\item Fonctionne sur tout ce qui supporte ```cpp std::get()``` et ```cpp std::tuple_size```
\item Notamment ```cpp std::pair``` et ```cpp std::array```
\end{itemize}
...
\begin{itemize}
\item ```cpp std::make_from_tuple()``` permet de construire un objet depuis un \textit{tuple-like}
\end{itemize}
...
#addproposal{P0220}{https://wg21.link/P0220}
...
\begin{itemize}
\item Ne peut pas contenir des références, des tableaux C, ```cpp void``` ni être vide
\end{itemize}
\end{alertblock}
...
\begin{itemize}
\item Testable via ```cpp operator bool()```
\item Accès à l'objet via ```cpp operator*```
\item Accès à un membre via ```cpp operator->```
\end{itemize}
\end{itemize}

#alertblock{Attention}
\begin{itemize}
\item ```cpp operator*``` ou ```cpp operator->``` indéfini sur un ```cpp std::optional``` vide
\end{itemize}
\end{alertblock}

\begin{itemize}
\item [] \begin{itemize}
\item ```cpp std::nullopt``` indique l'absence de l'objet
\end{itemize}
\item ```cpp value()``` retourne la valeur ou lève l'exception ```cpp std::bad_optional_access```
\item ```cpp value_or()``` retourne la valeur ou une valeur par défaut
\end{itemize}
...
\begin{itemize}
\item Y compris depuis un ```cpp std::initializer_list```
\end{itemize}
...
\begin{itemize}
\item Existence du \textit{helper} ```cpp std::make_optional```
\end{itemize}
...
== \mintinline[style=white]{cpp}```std::optional```}
\begin{itemize}
\item Changement de la valeur via ```cpp reset()```, ```cpp swap()```, ```cpp emplace()``` ou ```cpp operator=```
...
\begin{itemize}
\item En prenant en compte ```cpp std::nullopt```
\end{itemize}
...
\note[item]{```cpp std::nullopt``` est inférieur à toutes valeurs}
...
== \mintinline[style=white]{cpp}```std::optional```}
#alertblock{\mintinline[style=white]{cpp}```std::optional<bool>``` ? \mintinline[style=white]{cpp}```std::optional<T*>``` ?}
\begin{itemize}
\item Utilisez des booléens \og trois états\fg{} (```cpp Boost.tribool```)
...
#adviceblock{Do}
\begin{itemize}
\item Préférez ```cpp std::optional``` aux pointeurs bruts pour les données optionnelles
\end{itemize}
\end{exampleblock}

#noteblock{En attendant C++17}
\begin{itemize}
\item Utilisez ```cpp Boost.Optional```
\end{itemize}
\end{block}



== \mintinline[style=white]{cpp}```std::any```}
\begin{itemize}
\item ```cpp void*``` \textit{type-safe} contenant un objet de n'importe quel type (ou vide)
...
== \mintinline[style=white]{cpp}```std::any```}
\begin{itemize}
\item ```cpp any_cast<Type>()``` récupère la valeur
\item ... et lève une exception si le type demandé n'est pas correct
\end{itemize}
...
\item ... et retourne ```cpp nullptr``` si le type demandé n'est pas correct
\end{itemize}
...
\begin{itemize}
\item \textit{Helper} ```cpp std::make_any```
\end{itemize}
...
== \mintinline[style=white]{cpp}```std::any```}
\begin{itemize}
\item ```cpp reset()``` vide le contenu
\item ```cpp has_value()``` teste la vacuité
\item ```cpp type()``` récupère l'information du type courant

\note[item]{L'information de type récupérée ainsi est \textit{implementation-dependant} et non utilisable tel quel mais peut être comparé au retour de ```cpp type_id```}
\end{itemize}

#noteblock{En attendant C++17}
\begin{itemize}
\item Utilisez ```cpp Boost.Any```
\end{itemize}
\end{block}
...
== \mintinline[style=white]{cpp}```std::string_view```}
\begin{itemize}
...
\item Constructible depuis ```cpp std::string```, une chaîne C ou un pointeur et une taille
\end{itemize}

#alertblock{Attention !}
\begin{itemize}
\item Pas de {```cpp} \textbackslash```0} terminal systématique
...
== \mintinline[style=white]{cpp}```std::string_view```}
\begin{itemize}
\item ```cpp operator[]```, ```cpp at()```, ```cpp front()```, ```cpp back()```, ```cpp data()``` accèdent aux caractères
\item ```cpp remove_prefix()``` et ```cpp remove_suffix()``` modification les bornes
\item ```cpp size()``` et ```cpp length()``` accèdent à la taille
\item ```cpp max_size()``` accède à la taille maximale
\item ```cpp empty()``` teste la vacuité
\item ```cpp to_string()``` construction une chaîne depuis la vue
\item ```cpp copy()``` copie une partie de la vue
\item ```cpp substr()``` construit une vue sur une sous-partie
\item ```cpp compare()``` compare avec une autre vue ou une chaîne
\item ```cpp find()```, ```cpp rfind()```, ```cpp find_first_of()```, ```cpp find_last_of()```, ```cpp find_first_not_of()```, ```cpp find_last_not_of``` recherchent
\item ```cpp ==```, ```cpp !=```, ```cpp <=```, ```cpp >=```, ```cpp <``` et ```cpp >``` effectuent une comparaison lexicographique
\item ```cpp operator<<``` affiche la sous-chaîne
\end{itemize}
...
#adviceblock{Performances}
\begin{itemize}
\item Souvent meilleures que les fonctionnalités équivalentes de ```cpp string```
...
== Mémoire}
\begin{itemize}
\item ```cpp std::shared_ptr``` et ```cpp std::weak_ptr``` sur des tableaux
\end{itemize}
...
#alertblock{Pas de \mintinline[style=white]{cpp}```std::make_shared()```}
\begin{itemize}
\item ```cpp std::make_shared()``` ne supporte pas les tableaux en C++17
\end{itemize}
\end{alertblock}
...
#noteblock{Note}
\begin{itemize}
\item Pointeur intelligent sans responsabilité dans le TS ```cpp observer_ptr```
\item Mais pas dans le périmètre accepté pour C++17
\end{itemize}
\end{block}

#addproposal{P0414}{https://wg21.link/P0414}
#addproposal{P0035}{https://wg21.link/P0035}
...
\item ```cpp std::search()``` encapsule l'appel à un des foncteurs
\end{itemize}
\item Échantillonnage
\begin{itemize}
\item ```cpp std::sample()``` extrait aléatoirement n éléments d'un ensemble
\end{itemize}
\end{itemize}
...
== PGCD et PPCM}
\begin{itemize}
\item Ajout des fonctions ```cpp std::gcd()``` et ```cpp std::lcm()```
...
#addproposal{P0295}{https://wg21.link/P0295}
...
#addproposal{P0218}{https://wg21.link/P0218}
...
\begin{itemize}
\item Copie : ```cpp copy_file()```, ```cpp copy()```
\item Création de répertoires : ```cpp create_directory()```, ```cpp create_directories()```

\note[item]{Différence : création du répertoire final seulement ou de tous les répertoires intermédiaires nécessaires}

\item Création des liens : ```cpp create_symlink()```, ```cpp create_hard_link()```
\item Test d'existence : ```cpp exists()```
\item Taille : ```cpp file_size()```
\item Type : ```cpp is_regular_file()```, ```cpp is_directory()```, ```cpp is_symlink()```, ```cpp is_fifo()```, ```cpp is_socket()```, ...
\item Permissions : ```cpp permissions()```
\item Date de dernière écriture : ```cpp last_write_time()```
\item Suppression : ```cpp remove()```, ```cpp remove_all()```

\note[item]{Différence ```cpp remove``` et ```cpp remove_all``` : suppression récursive pour le second}

\item Changement de nom : ```cpp rename()```
\item Changement de taille : ```cpp resize_file()```
\item Chemin du répertoire temporaire : ```cpp temp_directory_path()```
\item Chemin du répertoire courant : ```cpp current_path()```
\end{itemize}
\end{itemize}
...
\begin{itemize}
\item Entrée du répertoire : ```cpp directory_entry```
\item Itérateurs pour le parcours
\begin{itemize}
\item Parcours simple : ```cpp directory_iterator```
\item Parcours récursif : ```cpp recursive_directory_iterator```
\end{itemize}
...
\end{itemize}
\item ```cpp std::fstream``` constructible depuis ```cpp path```
\end{itemize}
...
#noteblock{En attendant C++17}
\begin{itemize}
\item Utilisez ```cpp Boost.Filesystem```
\end{itemize}
\end{block}
...
#addproposal{P0024}{https://wg21.link/P0024}



== Parallelism TS}
\begin{itemize}
\item ```cpp std::for_each_n()``` parcours un ensemble défini par l'itérateur de début et sa taille

\note[item]{En soi, ce n'est pas lié au parallélisme mais cet algorithme apparait avec ce TS}

\item ```cpp std::reduce()``` \og ajoute\fg{} tous les éléments de l'ensemble
\end{itemize}

#noteblock{\mintinline[style=white]{cpp}```std::reduce()``` ou \mintinline[style=white]{cpp}```std::accumulate()``` ?}
\begin{itemize}
\item Ordre des \og additions\fg{} non spécifié dans le cas de ```cpp std::reduce()```
\end{itemize}

\note[item]{Le résultat n'est donc pas déterministe dans le cas d'opérations non associatives ou non commutatives (car ```cpp std::reduce()``` regroupe et réarrange les opérations)}
\note[item]{Mais rend cet algorithme parallélisable contrairement à ```cpp std::accumulate()```}
\end{block}
...
== Parallelism TS}
\begin{itemize}
\item ```cpp std::exclusive_scan()``` construit un ensemble où chaque élément est égal à la somme des éléments de rang strictement inférieur de l'ensemble initial et d'une valeur initiale
\end{itemize}
...
== Parallelism TS}
\begin{itemize}
\item ```cpp std::inclusive_scan()``` construit un ensemble où chaque élément est égal à la somme des éléments de rang inférieur ou égal de l'ensemble initial et d'une valeur initiale (si présente)
\end{itemize}
...
\note[item]{```cpp exclusive_scan``` et ```cpp inclusive_scan``` sont proche de ```cpp partial_sum``` mais sans garanti d'ordre des \og additions\fg{} avec résultat indéterminé en cas d'opérations non associatives (idem ```cpp reduce()``` / ```cpp accumulate()```)}
...
== Parallelism TS}
\begin{itemize}
\item ```cpp std::transform_reduce()``` : ```cpp std::reduce()``` sur des éléments préalablement transformés
\item ```cpp std::transform_exclusive_scan()``` : ```cpp std::exclusive_scan()``` sur des éléments préalablement transformés
\item ```cpp std::transform_inclusive_scan()``` : ```cpp std::inclusive_scan()``` sur des éléments préalablement transformés
\end{itemize}
...
#addproposal{P0226}{https://wg21.link/P0226}
