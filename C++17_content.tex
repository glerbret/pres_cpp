...
\begin{frame}[fragile]
	\frametitle{static\_assert sans message}
	\begin{itemize}
...
\subsection*{Types}
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::byte}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::map \& std::unordered\_map}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing emplace\_back(), emplace\_front()}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::to\_chars() et std::from\_chars}}
	\begin{itemize}
...
\subsection*{Variant}
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::variant}}
	\begin{itemize}
...
\note[item]{Le \mintinline{cpp}|std::variant| contient alors le premier type initialisé par défaut}
	...
	\begin{block}{\mintinline[style=vim]{cpp}|std::monostate|}
		\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Pack expansion sur using}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::uncaught\_exceptions()}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::invoke()}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::not\_fn()}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Attribut [[ fallthrough ]]}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Attribut [[ maybe\_unused ]]}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::shared\_mutex}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::scoped\_lock}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::apply()}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::optional}}
	\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::optional}}
	\begin{alertblock}{\mintinline[style=vim]{cpp}|std::optional<bool>| ? \mintinline[style=vim]{cpp}|std::optional<T*>| ?}
		\begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::any}}
	\begin{itemize}
...
\begin{frame}[fragile]
	...
	\begin{minted}{cpp}
		any a = 1;
		any_cast<int>(a);               // 1
		any_cast<bool>(a);              // Lance bad_any_cast
	\end{minted}
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::string\_view}}
	\begin{itemize}
	...
	\begin{alertblock}{Attention !}
		\begin{itemize}
			\item Pas de \mintinline{cpp}|0| terminal systématique
...
\begin{frame}[fragile]
	\frametitle{{\NoAutoSpacing std::string\_view}}
	\begin{itemize}
		\item Accès aux caractères : \mintinline{cpp}|operator[]|, \mintinline{cpp}|at()|, \mintinline{cpp}|front()|, \mintinline{cpp}|back()|, \mintinline{cpp}|data()|
		\item Modification des bornes : \mintinline{cpp}|remove_prefix()| et \mintinline{cpp}|remove_suffix()|
		\item Accès à la taille et à la taille maximale : \mintinline{cpp}|size()|, \mintinline{cpp}|length()| et \mintinline{cpp}|max_size()|
		\item Test de vacuité : \mintinline{cpp}|empty()|
		\item Construction d'une chaîne depuis la vue : \mintinline{cpp}|to_string()|
		\item Copie d'une partie de la vue : \mintinline{cpp}|copy()|
		\item Construction d'une vue sur une sous-partie de la vue : \mintinline{cpp}|substr()|
		\item Comparaison avec une autre vue ou une chaîne : \mintinline{cpp}|compare()|
		\item Recherche : \mintinline{cpp}|find()|, \mintinline{cpp}|rfind()|, \mintinline{cpp}|find_first_of()|, \mintinline{cpp}|find_last_of()|, \mintinline{cpp}|find_first_not_of()|, \mintinline{cpp}|find_last_not_of|
		\item Comparaison lexicographique : \mintinline{cpp}|==|, \mintinline{cpp}|!=|, \mintinline{cpp}|<=|, \mintinline{cpp}|>=|, \mintinline{cpp}|<| et \mintinline{cpp}|>|
		\item Affichage : \mintinline{cpp}|operator<<|
	\end{itemize}
\end{frame}
...
\begin{frame}[fragile]
	...
	\begin{alertblock}{Pas de \mintinline[style=vim]{cpp}|std::make_shared()|}
		\begin{itemize}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item Ajout des fonctions \mintinline{cpp}|gcd| et \mintinline{cpp}|lcm|
...
\begin{frame}[fragile]
	\frametitle{Parallelism TS}
	\begin{itemize}
		\item \mintinline{cpp}|std::for_each_n()| : variante de \mintinline{cpp}|std::for_each()| prenant l'itérateur de début et une taille et non une paire d'itérateurs
...
\note[item]{En soi, ce n'est pas lié au parallélisme mais cet algorithme apparait avec ce TS}
	...
	\begin{block}{\mintinline[style=vim]{cpp}|std::reduce()| ou \mintinline[style=vim]{cpp}|std::accumulate()| ?}
		\begin{itemize}
