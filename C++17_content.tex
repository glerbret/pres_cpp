...
\note[item]{Prendre garde à OCP malgré tout}
	\end{exampleblock}
\end{frame}
...
\note[item]{Voire \lstinline|char| ou \lstinline|signed char|, mais qui sont globalement de moins bon candidat pour un flux d'octet}
...
\note[item]{Fonctionne sur \lstinline|map| mais aussi \lstinline|multimap| et sur les versions unordered}
...
\note[item]{Dis autrement c'est du \textit{fold} (ou \textit{reduce}) appliqué en \textit{compile-time} sur des paramètres templates non-types}
	...
	\begin{center}
		\includegraphics[height=0.30\textheight]{input_src/C++/Left-fold-transformation.png} \includegraphics[height=0.30\textheight]{input_src/C++/Right-fold-transformation.png}
	\end{center}
\end{frame}
...
\note[item]{Comme tout objet en C++17 avec la déduction de type dans les constructeurs}
...
\note[item]{Pour le coup, là on est différent des pointeurs}
...
\note[item]{Mais dans la bibliothèque standard et non le langage}
...
\note[item]{Différence remove et remove\_all: suppression récursive pour le second}
...
\note[item]{Qt, POCO, etc. L'idée est d'avoir un code portable, mieux typé et moins source d'erreur}
