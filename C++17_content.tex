...
\note[item]{Pour les autres compilateurs, ça varie de \og rien\fg{} à \og très bon support\fg{} tant pour le langage que la bibliothèque}
...
\note[item]{Globalement un \lstinline|enum class| construit sur un \lstinline|unsigned char|}

		\item Supporte les opérations binaires (décalage, et, ou, non)
		\item Supporte les constructions depuis un type entier

\note[item]{Grâce à l'assouplissement des règles sur l'initialisation des \lstinline|enum class| en C++17}
...
\note[item]{Remplace avantageusement de la duplication de code, des fonctions complètement ad hoc, des désactivations abusives d'avertissement (voire des avertissements ignorés), etc.}
