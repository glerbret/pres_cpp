...
\note[item]{Le \lstinline|std::variant| contient alors le premier type initialisé par défaut}
		\end{itemize}
	\end{alertblock}
...
\begin{frame}[fragile]
	...
	\begin{lstlisting}[language=C++]
(pack op ...);  // right fold
(... op pack);  // left fold\end{lstlisting}
	...
	\begin{lstlisting}[language=C++]
(pack op ... op init);
(init op ... op pack);\end{lstlisting}
...
FoldPrint(10, 'a', "ert"s);\end{lstlisting}
	...
	\begin{itemize}
		\item Y compris \og \lstinline|,|\fg{} qui va donner une séquence d'actions
	\end{itemize}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item Utilisation possible de types différents pour \lstinline|begin| et \lstinline|end|
...
\note[item]{Maintenant, il ne le peut plus et doit seulement ignorer l'attribut (éventuellement avec un avertissement)}
		...
		\item \lstinline|using| des attributs non standards
	\end{itemize}
...
\begin{frame}[fragile]
	...
	\begin{exampleblock}{Do}
		\begin{itemize}
			\item Préférez \lstinline|std::optional| aux pointeurs bruts pour les données optionnelles
		\end{itemize}
