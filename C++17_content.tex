...
\begin{frame}[fragile]
	...
	\begin{block}{Note}
		Les branches doivent être syntaxiquement correctes mais pas nécessairement sémantiquement valides
	\end{block}
	...
	\begin{exampleblock}{Do}
		Préférez \lstinline|if constexpr| aux suites de spécialisations de template et SFINAE, aux imbrications de ternaires ou à \lstinline|#if|
	\end{exampleblock}
\end{frame}
...
\begin{frame}[fragile]
	...
	\begin{block}{Objectif}
		...
		Remplacement de \lstinline|std::tie()|
	\end{block}
...
{ ... }\end{lstlisting}
	...
	\begin{block}{Motivation}
		Comportement différent d'un destructeur en présence d'exception (p.ex. \textit{rollback})
	\end{block}
\end{frame}
...
\frametitle{Déduction de template dans les constructeurs\titlehfill{}3/3}
	\begin{block}{Note}
		Rend obsolète plusieurs \textit{make helper} (\lstinline|make_pair|, \lstinline|make_tuple|, \ldots)
	\end{block}
...
invoke(&Foo::bar, foo, 8);  // 50\end{lstlisting}
	...
	\begin{block}{Motivation}
		Syntaxe unique d'appel d'appelable
	\end{block}
\end{frame}
...
\begin{frame}[fragile]
	\frametitle{\lstinline|std::basic_string_view|\titlehfill{}1/3}
	\begin{itemize}
...
\note[item]{Gestion de l'alignement des \textit{Over-Aligned Data}}
	...
	\begin{block}{Note}
		Présence dans le TS d'un pointeur intelligent sans responsabilité (observateur) : \lstinline|observer_ptr|, mais pas dans le périmètre accepté pour C++17
	\end{block}
\end{frame}
