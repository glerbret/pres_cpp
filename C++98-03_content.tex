...
\note[item]{Tendance des langages industriels \textit{mainstream} (Java - lambda, \lstinline|generic|, code dans les interfaces-, C\#, Php, Python, ECMAScript)}

		\item Essentiellement impératif à typage statique déclaratif
...
\note[item]{\textit{General Purpose} : par opposition à Fortran, Cobol, R ou l'assembleur}
		...
		\item À l'origine, ajout des classes (issues Simula) au C
...
\note[item]{Trop proche pour certains, pas assez pour d'autres qui souhaiteraient que le sous-ensemble commun soit le C}
...
\note[item]{meilleur typage que C (à l'origine d'incompatibilités : caractères littéraux de type \lstinline|char|, pas de conversion implicite depuis \lstinline|void|)}
		...
		\item \textit{Zero-overhead abstraction} : \og Ne payons pas pour ce que nous n'utilisons pas\fg{}
		\item Compatibilité ascendante forte mais pas absolue

\note[item]{Il y a eu des ruptures au passage à C++11 (et aux versions suivantes)}

		\item Évolutions par les bibliothèques plutôt que par le langage

\note[item]{En particulier la bibliothèque standard}

		\item Pas de \og magie\fg{} dans la bibliothèque standard

\note[item]{Le contenu de la bibliothèque standard pourrait être écrit dans une bibliothèque tierce (contrairement aux \lstinline|string| java)}
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item \ldots{} mais recommandé d'hériter de \lstinline|std::exception| via \lstinline|std::logic_error|, \lstinline|std::runtime_error| ou autres
...
\note[item]{Cette destruction se fait dans l'ordre inverse de la création des objets}
...
\note[item]{\textit{No-throw} : les exceptions internes sont attrapées et traitées en interne}
...
\note[item]{Via \lstinline|std::logic_error| ou \lstinline|std::runtime_error| plutôt que directement}
...
\note[item]{Par exemple l'absence d'un élément dans une collection lors de la recherche}

			\item Réservez les exceptions au signalement d'erreurs

\note[item]{Donc pas pour la gestion locale des erreurs non plus}
...
\note[item]{Problème rare qui concerne peu d'objets et uniquement dans des cas extrêmes}
		\end{itemize}
	\end{alertblock}
...
\note[item]{\lstinline|std::auto_ptr| est déprécié en C++11 et remplacé par \lstinline|std::unique_ptr| qui fonctionne mieux (sémantique de déplacement)}
...
\note[item]{Permet de mettre en place RAII et de limiter les accès non contrôlés et pertes de responsabilité}
	...
	\begin{itemize}
		...
		\item \ldots{} mais ne doit pas utiliser \lstinline|B| pour accéder à un troisième objet
...
\note[item]{Interface étendue : opérateur d'IO, de calcul, \ldots}
...
\note[item]{Support GC meilleur en C++11 mais pas parfait, non implémenté par les compilateurs et qui risquent d'être supprimé}
...
\note[item]{Via les itérateurs donc sans connaissance du conteneur ni de la structure de donnée}
...
\note[item]{Stepanov a de vrais arguments et cible davantage, à mon avis, le marketing survendeur (\textit{silver bullet}) et les promesses non tenues que la POO elle-même}
...
\note[item]{\lstinline|std::valarray| est laissé de côté, pas vraiment un conteneur}
...
\note[item]{\textit{Ropes} : chaîne découpée en morceaux reliés par une liste ou un arbre}
\note[item]{\textit{Ropes} : performance de modification au milieu, de découpage, etc., permettent le partage de sous-chaînes, résolvent le problème d'\textbf{espace d'adressage} pas assez grand malgré une mémoire libre largement suffisante, certaine localité des données permettant de bénéficier des lignes de caches (par rapport à une liste)}
\note[item]{\textit{Ropes} : alternative possible aux \lstinline|std::vector| même si \lstinline|std::deque| rempli déjà en partie ce rôle}
\note[item]{En toute rigueur, \lstinline|std::string| pouvait être implémenter sous forme de \textit{ropes} en C++98/03, plus en C++11}
...
\note[item]{Insertion en vrac : \lstinline|std::vector| vs. \lstinline|std::list|}
...
\note[item]{Appelés aussi \textit{functor} en anglais on foncteur en français}
		\end{itemize}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item \lstinline|std::lower_bound()| retourne un itérateur sur le première élément non strictement inférieur à la valeur recherchée
