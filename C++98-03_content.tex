...
\note[item]{Également une FAQ plutôt bien sur C++, ainsi qu'un guide de bonnes pratiques, les brouillons de la norme et des propositions}
	...
	\begin{itemize}
		\item Un repo GIT (brouillons et propositions) : \url{https://github.com/cplusplus}
...
\note[item]{Destruction dans l'ordre inverse de la création des objets}
		...
		\item Exceptions non attrapées provoquent l'appel de \lstinline|std::terminate()|
...
\begin{frame}[fragile]
	...
	\begin{alertblock}{\lstinline|std::auto_ptr|}
		\begin{itemize}
			\item Copiable
			\item Copie transfère la responsabilité de la ressource
		\end{itemize}
	\end{alertblock}
...
\note[item]{Pas d'implémentation non contigüe connue}
			...
			\item Variante pour les caractères larges : \lstinline|std::wstring|
		\end{itemize}
	\end{itemize}
...
\note[item]{Ce qui représente un avantage sur les fonctions libres}
		...
		\item Foncteur standards : \lstinline|std::plus|, \lstinline|std::minus|, \lstinline|std::equal|, \lstinline|std::less|, \ldots{}
		\item Constructibles
		\begin{itemize}
...
\begin{frame}[fragile]
	...
	\begin{block}{Variantes \og \lstinline|_copy|\fg{}}
		\begin{itemize}
			\item Algorithmes suffixés par \lstinline|_copy| fonctionnent comme l'algorithme de base en troquant la modification en place contre une copie du résultat
		\end{itemize}
	\end{block}
