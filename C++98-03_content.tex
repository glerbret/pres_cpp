...
\note[item]{Les Technical Specification (TS) sont des documents normatifs ayant vocation a être intégrés dans la norme, ce sont essentiellement des \textit{preview}}
...
\note[item]{Tendance des langages industriels mainstream (Java - lambda, generic, code dans les interfaces-, C\#, Php, Python, ECMAScript)}
...
\note[item]{Le problème se résoud en forçant l'écriture (flush) en fin de la fonction \lstinline|copy_files()| et en remontant une exception en cas d'erreur}
\end{frame}
...
\note[item]{Inadaptation à d'autres types de ressource est la raison d'être du try with resource de Java}
...
\note[item]{Pourquoi OMA : les soft demandent plus de mémoire que ce qu'ils utilisent réellement}
...
\subsection*{STL}
\begin{frame}
	\frametitle{STL - \textit{Standard Template Library}\titlehfill{}1/3}
	\begin{itemize}
...
\note[item]{Stepanov a de vrais arguments et cible davantage, amha, le marketing survendeur (silver bullet) et les promesses non tenues que la POO elle-même}
...
\note[item]{\textit{Ropes} : alternative possible aux vector même si deque rempli déjà en partie ce rôle}
\note[item]{En toute rigueur, \lstinline|std::string| pouvait être implémenter sous forme de ropes en C++98/03, plus en C++11}
...
\note[item]{Appelés aussi functor en anglais on foncteur en français}
		\end{itemize}
