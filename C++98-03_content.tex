...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item Lancées par \lstinline|throw|
		\item Attrapées par \lstinline|catch()| depuis un bloc \lstinline|try|
	\end{itemize}
...
\note[item]{Destruction dans l'ordre inverse de la création des objets}
		...
		\item Appel de \lstinline|std::terminate()| si une exception n'est pas attrapée
...
\note[item]{Problème résolu en forçant l'écriture (\lstinline|flush|) en fin de la fonction \lstinline|copy_files()| et en remontant une exception en cas d'erreur}
\end{frame}
...
\note[item]{Formellement, la STL n'existe plus dans la norme C++ actuelle}
		...
		\begin{itemize}
			\item Conteneurs et \lstinline|std::basic_string|
...
\note[item]{Pas d'implémentation non contigüe connue}
			...
			\item Variante \lstinline|std::wstring| pour les caractères larges
		\end{itemize}
	\end{itemize}
...
\note[item]{\lstinline|std::valarray| laissé de côté, pas vraiment un conteneur}
	...
	\begin{block}{\lstinline|std::bitset| vs. \lstinline|std::vector<bool>|}
		Objectif de gain mémoire adressé par \lstinline|std::bitset|, pourquoi \lstinline|std::vector<bool>| n'est-il pas un vrai conteneur de booléen ?
	\end{block}
...
\note[item]{\lstinline|const_iterator| itèrent sur des valeurs constantes, utilisables pour lire des données mais pas pour les écrire}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item Instances de classe définissant \lstinline|operator()|
	\end{itemize}
...
\note[item]{Ce qui représente un avantage sur les fonctions libres}
		...
		\item Foncteurs standards : \lstinline|std::plus|, \lstinline|std::minus|, \lstinline|std::equal|, \lstinline|std::less|, \ldots{}
		\item Constructibles
		\begin{itemize}
			\item Depuis des pointeurs de fonctions : \lstinline|std::prt_fun|
			\item Depuis des fonctions membres : \lstinline|std::mem_fun|, \lstinline|std::mem_fun1|, \ldots{}
			\item En niant d'autres foncteurs : \lstinline|std::not1|, \lstinline|std::not2|
			\item En fixant des paramètres : \lstinline|std::bind1st|, \lstinline|std::bind2nd|
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}[fragile]
	...
	\begin{block}{Variantes \og \lstinline|_if|\fg{}}
		\begin{itemize}
			\item Les algorithmes suffixés par \lstinline|_if| utilisent un prédicat
		\end{itemize}
	\end{block}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item \lstinline|std::equal_range()| retourne la paire (\lstinline|std::lower_bound|, \lstinline|std::upper_bound|)
	\end{itemize}
...
foo(bar, 12);  // vec.end
foo(bar, 13);  // iterateur sur 13\end{lstlisting}
...
\begin{frame}[fragile]
	...
	\begin{block}{Variantes \og \lstinline|_copy|\fg{}}
		\begin{itemize}
			\item Les algorithmes suffixés par \lstinline|_copy| fonctionnent comme l'algorithme de base en troquant la modification en place contre une copie du résultat
		\end{itemize}
	\end{block}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item \lstinline|std::remove()| élimine les éléments égaux à une valeur donnée
	\end{itemize}
...
\note[item]{\lstinline|std::remove()| ne supprime pas car les algorithmes ne peuvent pas modifier le conteneur, en particulier sa taille, mais seulement son contenu}
...
\begin{frame}[fragile]
	\frametitle{STL Algorithmes -- Suppression}
	\begin{itemize}
		\item \lstinline|std::remove_if()| élimine les éléments vérifiant le prédicat
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item \lstinline|std::unique()| élimine les éléments consécutifs égaux sauf le premier
	\end{itemize}
...
\note[item]{Les complexités moyennes ne sont pas pertinentes pour des données particuliers, p.ex. tri par insertion efficace sur ensembles presque triés, peu de permutation du tri par sélection donc efficace pour des données lourdes à déplacer}
	...
	\begin{exampleblock}{Do}
		\begin{itemize}
			\item Préférez les algorithmes standards aux algorithmes tierces et \og maisons\fg{}
		\end{itemize}
	\end{exampleblock}
