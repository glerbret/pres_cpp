...
\begin{frame}[fragile]
	\frametitle{Ressources -- Limites du RAII}
	\begin{alertblock}{\mintinline{cpp}|std::auto_ptr|}
		\begin{itemize}
			\item Copiable
...
\note[item]{Passage à un tableau C en prenant l'adresse du premier élément : \mintinline{cpp}|T* ptr = &vec[0];|}
	...
	\begin{alertblock}{\mintinline{cpp}|std::vector<bool>| n'est pas un vecteur de booléen}
		\begin{itemize}
...
\begin{frame}[fragile]
	...
	\begin{alertblock}{\mintinline{cpp}|std::string| et UTF-8}
		\begin{itemize}
...
\note[item]{\mintinline{cpp}|std::valarray| laissé de côté, pas vraiment un conteneur}
	...
	\begin{block}{\mintinline[style=vim]{cpp}|std::bitset| vs. \mintinline[style=vim]{cpp}|std::vector<bool>|}
...
\begin{frame}[fragile]
	\frametitle{STL Conteneurs -- std::pair}
	\begin{itemize}
...
\begin{frame}[fragile]
	...
	\begin{block}{Variantes \og \mintinline{cpp}|_if|\fg{}}
		\begin{itemize}
...
\note[item]{C++14 ajoute des surcharges de \mintinline{cpp}|std::equal()| testant la taille}
	...
	\begin{block}{Et \mintinline[style=vim]{cpp}|operator==| ?}
		\begin{itemize}
...
\begin{frame}[fragile]
	...
	\begin{block}{Variantes \og \mintinline{cpp}|_copy|\fg{}}
		\begin{itemize}
...
\note[item]{\mintinline{cpp}|std::remove()| ne supprime pas car les algorithmes ne peuvent pas modifier le conteneur, en particulier sa taille, mais seulement son contenu}
	...
	\begin{block}{Idiome \textit{Erase-Remove}}
		Suppression via un appel à \mintinline{cpp}|erase()| sur les éléments après le nouvel itérateur de fin
