...
\note[item]{C'est de moins en moins vrai, de plus en plus de sujet rentre directement dans les sujets du comité sans passer par Boost}
...
\note[item]{Contrairement à la bibliothèque standard, Boost n'a pas pu créé une surcharge de \lstinline|std::fstream| prenant en paramètre un chemin (pas de modification de \lstinline|std| hors de la bibliothèque standard)}
	\end{itemize}
...
\begin{frame}[fragile]
	\frametitle{Boost.Format}
	\begin{itemize}
		\item Alternative intégrée à C++20

\note[item]{Alternative, car cette fois, ce n'est pas Boost.format qui a été repris mais une autre bibliothèque}
...
\begin{frame}[fragile]
	\frametitle{Boost.StringAlgorithms}
	\begin{itemize}
